...
std::string("tcp://").append(
                
getenv("CACHE_HOST")).append(":").append(std::getenv("CACHE_PORT"));
...
std::optional<std::string> UserCache::getUserFromCache(const std::string& 
login) {
    return cache_.get(login);
}

void UserCache::addUserToCache(const std::string& login, const 
std::string& userInfo) {
